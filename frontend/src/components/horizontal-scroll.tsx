...
function Card({ card, index }: { card: Card; index: number }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -10 }}
      className="relative flex-shrink-0 w-[450px] h-[600px] group cursor-pointer"
      style={{ width: "450px" }}
    >
      {/* Premium glass card with subtle glow */}
      <div className="relative h-full p-[1px] rounded-[32px]">
        ...
        {/* Main card */}
        <div className="relative h-full bg-slate-950 border border-cyan-500/20 rounded-[31px] p-8 flex flex-col backdrop-blur-xl shadow-[0_0_40px_rgba(6,182,212,0.1)] group-hover:shadow-[0_0_60px_rgba(6,182,212,0.25)] transition-all duration-500">
          
          {/* Subtle gradient background */}
          <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/5 to-transparent rounded-[31px] opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
          
          {/* Icon - similar to Semantic Analysis style */}
          <div className="relative w-20 h-20 rounded-full bg-cyan-500/10 border border-cyan-400/40 mb-6 flex items-center justify-center shadow-[0_0_30px_rgba(6,182,212,0.3)]">
            ...
            <motion.div 
              ...
              className="absolute inset-[-4px] rounded-full border-2 border-dashed border-cyan-500/30 group-hover:border-cyan-400/60 transition-colors"
            />
          </div>

          {/* Title */}
          <h3 className="text-3xl font-bold text-white mb-2 tracking-tight relative z-10">
            {card.title}
          </h3>
          <p className="text-cyan-400 font-semibold mb-3 relative z-10">
            {card.subtitle}
          </p>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6 relative z-10">
            {card.stats.map((stat, i) => (
              <motion.div 
                key={i} 
                className="bg-slate-900/50 backdrop-blur-xl rounded-xl p-4 border border-white/5 group-hover:border-cyan-500/20 transition-colors"
          ...
          <div className="flex-grow relative z-10">
            <div className="space-y-3">
              {card.features.map((feature, i) => (
                ...
                >
                  <div className="w-5 h-5 rounded-full bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center flex-shrink-0 group-hover/item:bg-cyan-500/20 group-hover/item:border-cyan-400/50 transition-colors">
