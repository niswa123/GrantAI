...
export default function SettingsLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex text-slate-100">
      ...
      {/* ── Mobile top bar ── */}
      <div className="md:hidden fixed top-14 left-0 right-0 z-30 bg-slate-950/90 backdrop-blur-xl border-b border-white/8 flex items-center gap-1.5 px-3 py-2 overflow-x-auto scrollbar-hide">
        <Link
          href="/dashboard"
          className="flex items-center gap-1 px-2.5 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap text-slate-500 hover:text-white border border-transparent shrink-0 touch-manipulation"
        >
        ...
        {settingsNav.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all shrink-0 touch-manipulation ${
                active
